...
public class RecipeController {
	...
	public ResponseEntity<Map<String, Object>> getRecipes(@RequestBody Map<String, Object> mapOfFilters){
		Map<String, Object> mapForReponse = new HashMap<>();
		
		if(mapOfFilters.isEmpty()){
			List<Recipe> listOfRecipe = recipeService.getAllRecipe(-1,-1);
			listOfRecipe.forEach(x -> converRecipeToJson(x));
			mapForReponse.put("recipes", listOfRecipe);
			mapForReponse.put("success", true);
		} else if(mapOfFilters.get("page") != null && mapOfFilters.get("size") != null){
			List<Recipe> listOfRecipe = recipeService.getAllRecipe((int) mapOfFilters.get("page"), (int)mapOfFilters.get("size"));
			listOfRecipe.forEach(x -> converRecipeToJson(x));
			mapForReponse.put("recipes", listOfRecipe);
			mapForReponse.put("success", true);
		} else 
